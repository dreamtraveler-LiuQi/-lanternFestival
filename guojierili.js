...
function daojishi() {
    ...
    let interval = setInterval(function () {
        let juli = dateDifference('2022-02-15 00:00:00');
        let aa = juli.substring(0, juli.indexOf('分') + 1);
        let bb = juli.substring(juli.indexOf('分') + 1, juli.indexOf('秒'));
        let tempmiao = "<span class='my-face'>" + bb + "</span>";
        document.getElementById("juli").innerHTML = aa + tempmiao + "秒";
        let timeSh = juli.substring(juli.indexOf('时') + 1, juli.indexOf('分'));
        if (+timeSh === 0 || +timeSh < 0) {
            console.log("++++++yes", +timeSh);
            clearInterval(interval);
            document.getElementById("juli").innerHTML = "0时 0分 0秒";
            window.location.href = "./endTime/index.html"
        } else {
            console.log("++++++on", +timeSh);
        }
